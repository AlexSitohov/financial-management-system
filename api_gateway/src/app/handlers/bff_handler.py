...
class BFFHandler:
    ...
    @http_exc_handler
    async def reroute_to_appropriate_service(self, request: Request) -> JSONResponse | Response:
...
def _json_response(body: bytes, status_code: int, headers: dict[str, str]) -> FastAPIResponse:
    return FastAPIResponse(content=body, status_code=status_code, headers=headers)

def _default_response(body: bytes, status_code: int, headers: dict[str, str]) -> FastAPIResponse:
    return FastAPIResponse(
        content=body.decode("utf-8") if isinstance(body, bytes) else body, status_code=status_code, headers=headers
    )


response_mapper = {
    "application/json": _json_response
}
